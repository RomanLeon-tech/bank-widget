...
def test_log_error_to_file(tmp_path, x, y):
    ...
    with open(log_file, "r") as f:
        content = f.read().strip()
        expected_content = (f"my_function error: "
                            f"ValueError. Inputs: ({x}, {y}), {{}}")
...
def test_log_error_to_console(capsys, x, y):
    ...
    with pytest.raises(ValueError):
        my_function(x, y)
    captured = capsys.readouterr()
    assert (f"my_function error: ValueError. "
            f"Inputs: ({x}, {y}), {{}}") in captured.err
